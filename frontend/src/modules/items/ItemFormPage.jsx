...
const defaultValues = {
  ...
  sectionId: '', categoryId: '', subCategoryId: '', subSubCategoryId: '', categoryPath: '', defaultWarehouse: '', reorderLevel: 0, reorderQty: 0, openingStock: 0, openingStockRate: 0,
...
function ItemFormPage({ mode = 'edit' }) {
  ...
  const seasons = useSelector((state) => state.masters?.seasons || []);
  ...
  const onSubmit = async (data, statusOverride = data.status || 'Active', stay = false) => {
    console.log('Items Form Submit Data:', data);
  ...
  const fields = {
    basic: [['itemCode', 'Style Code *'], ['itemName', 'Item Name *'], ['shadeColor', 'Color / Shade'], ['uom', 'UOM']],
    attrs: [['fabric', 'Fabric'], ['pattern', 'Pattern'], ['fit', 'Fit'], ['gender', 'Gender']],
  };
      ...
      <Paper elevation={0} sx={{ border: '1px solid #e2e8f0', borderRadius: 2, mb: 2, overflow: 'hidden' }}>
        ...
        <Box sx={{ p: 3 }}>
            ...
            <Stack spacing={3}>
              <FormSection title="Core Information" subtitle="Item identity and classification.">
                <Grid container spacing={2}>
                  {fields.basic.map(([key, label]) => (
                    <Grid key={key} item xs={12} md={key === 'itemName' ? 6 : 3}>
                      <TextField fullWidth size="small" label={label} {...register(key, { required: key === 'itemCode' || key === 'itemName' ? `${label} is required.` : false })} error={Boolean(errors[key])} helperText={errors[key]?.message || ' '} disabled={isViewMode} />
                    </Grid>
                  ))}
                  ...
                  <Grid item xs={12} md={3}>
                    <TextField fullWidth size="small" select label="Status" {...register('status')} disabled={isViewMode}>
              ...
              <FormSection title="Category Hierarchy" subtitle="Organize item in Section > Category > Sub Group > Style.">
                <Grid container spacing={2}>
                  <Grid item xs={12} md={3}>
                    <TextField fullWidth size="small" select label="Section / Gender *" {...register('sectionId', { required: 'Section is required.' })} error={Boolean(errors.sectionId)} helperText={errors.sectionId?.message || ' '} disabled={isViewMode}>
                      <MenuItem value="">Select Section</MenuItem>
                      {itemGroups.filter(g => (g.groupType === 'Section' || !g.parentId) && !['Brand', 'Season'].includes(g.groupType)).map(g => <MenuItem key={g.id || g._id} value={g.id || g._id}>{g.groupName || g.name}</MenuItem>)}
                    </TextField>
                  </Grid>
                  <Grid item xs={12} md={3}>
                    <TextField fullWidth size="small" select label="Category *" {...register('categoryId', { required: 'Category is required.' })} error={Boolean(errors.categoryId)} helperText={errors.categoryId?.message || ' '} disabled={isViewMode}>
                      <MenuItem value="">Select Category</MenuItem>
                      {itemGroups.filter(g => g.parentId === watch('sectionId')).map(g => <MenuItem key={g.id || g._id} value={g.id || g._id}>{g.groupName || g.name}</MenuItem>)}
                    </TextField>
                  </Grid>
                  <Grid item xs={12} md={3}>
                    <TextField fullWidth size="small" select label="Sub Group" {...register('subCategoryId')} disabled={isViewMode}>
                      ...
                      {itemGroups.filter(g => g.parentId === watch('categoryId')).map(g => <MenuItem key={g.id || g._id} value={g.id || g._id}>{g.groupName || g.name}</MenuItem>)}
                    </TextField>
                  </Grid>
                  <Grid item xs={12} md={3}>
                    <TextField fullWidth size="small" select label="Style / Type" {...register('subSubCategoryId')} disabled={isViewMode}>
                      ...
                      {itemGroups.filter(g => g.parentId === watch('subCategoryId')).map(g => <MenuItem key={g.id || g._id} value={g.id || g._id}>{g.groupName || g.name}</MenuItem>)}
                    </TextField>
                  </Grid>
                </Grid>
              </FormSection>

              <FormSection title="Descriptors" subtitle="Garment specific attributes for better search and reporting.">
                <Grid container spacing={2}>
                  {fields.attrs.map(([key, label]) => (
                    <Grid key={key} item xs={12} md={2.4}>
                      <TextField fullWidth size="small" label={label} {...register(key)} disabled={isViewMode} />
                    </Grid>
                  ))}
                  <Grid item xs={12}>
          ...
          <Box sx={{ display: activeTab === 'variants' ? 'block' : 'none' }}>
            <VariantTable variants={variants} onChange={(updated) => { setVariantError(''); setVariants(updated); }} styleCode={styleCode || 'ITEM'} readOnly={isViewMode} sizeOptions={sizes.map(s => s.sizeCode)} />
          </Box>
          <Box sx={{ display: activeTab === 'media' ? 'block' : 'none' }}>
            <FormSection title="Media" subtitle="Frontend-ready image slots for item presentation.">
              <Grid container spacing={2}>{images.map((img, index) => <Grid item xs={12} sm={6} md={4} key={index}><Paper elevation={0} sx={{ border: '1px dashed #cbd5e1', borderRadius: 2, p: 1.5, minHeight: 160, display: 'flex', alignItems: 'center', justifyContent: 'center', bgcolor: '#f8fafc', position: 'relative', overflow: 'hidden' }}>{img?.preview ? <><Box component="img" src={img.preview} sx={{ width: '100%', height: 160, objectFit: 'cover', borderRadius: 1 }} />{!isViewMode ? <Button size="small" color="error" variant="contained" onClick={() => removeImage(index)} sx={{ position: 'absolute', top: 8, right: 8, minWidth: 0, px: 1 }}>X</Button> : null}</> : <Button component="label" sx={{ textTransform: 'none', color: '#64748b' }} disabled={isViewMode}><Stack spacing={1} sx={{ alignItems: 'center' }}><UploadFileIcon sx={{ fontSize: 32 }} /><Typography variant="body2" sx={{ fontWeight: 700 }}>{index === 0 ? 'Main Image' : `Additional Image ${index}`}</Typography></Stack><input hidden type="file" accept="image/*" onChange={handleImageChange(index)} /></Button>}</Paper></Grid>)}</Grid>
            </FormSection>
          </Box>
          <Box sx={{ display: activeTab === 'inventory' ? 'block' : 'none' }}>
            <FormSection title="Inventory Defaults" subtitle="Defaults used by warehouse operations, barcode printing, and replenishment.">
              <Grid container spacing={2}>
