...
const baseSupplierFields = [
  ...
  { name: 'supplierCode', label: 'Supplier Code', required: true },
  {
    ...
    required: true,
    pattern: { value: /^[A-Za-z\s.]+$/, message: 'Contact person can only contain letters and spaces' },
  },
  {
    name: 'phone',
    label: 'Phone',
  ...
  },
  {
    ...
    validate: (v, formValues) => {
      if (!v) return true;
      if (!/^\d{10}$/.test(v)) return 'Alternate phone must be exactly 10 digits';
      if (v === formValues?.phone) return 'Alternate phone cannot be the same as primary phone';
      return true;
    },
  },
  {
  ...
  },
  {
    ...
    label: 'GST Number',
    required: true,
    pattern: { value: /^[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]{1}[1-9A-Z]{1}Z[0-9A-Z]{1}$/, message: 'Invalid 15-char GSTIN format' },
  },
  {
    name: 'panNo',
    label: 'PAN No',
    pattern: { value: /^(?:[A-Z]{5}[0-9]{4}[A-Z]{1})?$/, message: 'Invalid 10-char PAN format' },
  },
  ...
  { name: 'pincode', label: 'Pincode', required: true, pattern: { value: /^\d{6}$/, message: 'Pincode must be exactly 6 digits' } },
  {
    ...
    required: true,
    defaultValue: 'India',
    pattern: { value: /^[A-Za-z\s]+$/, message: 'Country can only contain alphabets and spaces' },
  },
