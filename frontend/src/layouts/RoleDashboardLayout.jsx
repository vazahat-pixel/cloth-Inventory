...
function RoleDashboardLayout() {
  ...
  return (
    <Box sx={{ minHeight: '100vh', display: 'flex', backgroundColor: '#f4f6f8' }}>
      <RoleSidebar navConfig={navConfig} />
      <Box sx={{ width: 240, flexShrink: 0 }} aria-hidden />
      <Box sx={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <Topbar />
        <Box component="main" sx={{ flex: 1, width: '100%', minWidth: 0, overflowY: 'auto', overflowX: 'hidden', p: { xs: 2, sm: 3 } }}>
          <Outlet />
        </Box>
      </Box>
    </Box>
