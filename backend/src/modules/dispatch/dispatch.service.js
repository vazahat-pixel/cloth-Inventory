...
const updateDispatch = async (id, dispatchData, userId) => {
    return await withTransaction(async (session) => {
        ...
        // Update the Dispatch Master
        dispatchMaster.items = enrichedItems;
...
const confirmDispatch = async (id, userId) => {
    return await withTransaction(async (session) => {
        ...
        for (const item of dispatch.items) {
            // Resolve barcode and itemId from Item master
            const { barcode, itemId } = await resolveVariantInfo(item.variantId, session);
...
const receiveDispatch = async (id, userId, receivedItems = []) => {
    return await withTransaction(async (session) => {
        ...
        for (const item of dispatch.items) {
            // Resolve barcode and itemId from Item master
            const variantId = item.variantId?._id || item.variantId;
            const { barcode, itemId } = await resolveVariantInfo(variantId, session);

            // 1. ALWAYS clear the FULL dispatched qty from in-transit (regardless of audit qty)
            //    But gracefully handle if in-transit record doesn't exist yet (edge case)
            try {
                ...
                console.warn(`[RECEIVE-WARN] Could not remove from in-transit (may not exist): ${err.message}`);
            }

            // 2. Add VERIFIED quantity to physical store inventory
            //    If audit payload provided, use it; otherwise use dispatched qty
            const verified = (receivedItems || []).find(ri => String(ri.variantId) === String(variantId));
            ...
            if (qtyToReceive > 0) {
                ...
                if (relabelOnTransfer) {
                    ...
                    const storeCode = store ? (store.storeCode || 'STR') : 'STR';
                    targetBarcode = `${storeCode}-${barcode}`;
                    console.log(`[RELABEL] New Barcode for Store: ${targetBarcode}`);
                }

                // Landing cost = rate + tax per unit
                const taxPerUnit = item.qty > 0 ? ((item.taxAmount || 0) / item.qty) : 0;
                const landingCost = (item.rate || 0) + taxPerUnit;
