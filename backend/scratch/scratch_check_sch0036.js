...
async function run() {
    try {
        ...
        const disp = await Dispatch.findById('6a366ad36aa096db0c8642be');
        if (disp) {
            ...
            disp.items.forEach(it => {
                console.log(`  - Item Name: ${it.itemName || 'N/A'}, barcode: ${it.barcode}, qty: ${it.qty}, rate: ${it.rate}, hsnCode: ${it.hsnCode}`);
            });
